...
export default async function TalentMessagesPage() {
  ...
  const { data: conversations } = await supabase
    .from("conversations")
    .select("id, hirer_id, gig_id, created_at")
    .eq("talent_id", user.id)
    .order("created_at", { ascending: false });

  const hirerIds = [...new Set((conversations ?? []).map((c) => c.hirer_id))];
  const gigIds = [...new Set((conversations ?? []).map((c) => c.gig_id))];
  const conversationIds = (conversations ?? []).map((c) => c.id);

  const [{ data: hirerProfiles }, { data: gigs }, { data: messages }] = await Promise.all([
    hirerIds.length
      ? supabase.from("hirer_profiles").select("user_id, company_name").in("user_id", hirerIds)
  ...
  const companyByHirer = new Map((hirerProfiles ?? []).map((h) => [h.user_id, h.company_name]));
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      {!conversations?.length ? (
        <EmptyState
          ...
          description="Once you apply to a job, you can message the hirer here."
        />
      ) : (
        <div className="space-y-3">
          {conversations.map((c) => {
            ...
            return (
              <Link
                key={c.id}
                href={`/dashboard/talent/messages/${c.id}`}
                className={`block p-5 ${glassCardLight} ${glassCardLightHover}`}
              >
                <div className="flex items-center justify-between">
                  <p className="font-semibold text-ink-navy">{companyByHirer.get(c.hirer_id) ?? "Hirer"}</p>
                  {last && (
